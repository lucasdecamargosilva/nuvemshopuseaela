(function () {
    ...
    (function() {
        function injectPLBadge() {
            try {
                ...
                a.href = 'https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=useaela';
                a.target = '_blank';
                a.rel = 'noopener';
    ...
    // ===============================================
    const apiKey = "pl_live_8531d8b6021feb9d1a1173982330c8c6ef750717ba280a7da7b7640c578241c5";
    ...
    const WEBHOOK_CHECK_LIMIT = 'https://n8n.segredosdodrop.com/webhook/useaela-check-limit';
    ...
    const html = `
        <div id="q-modal-ia">
            <div class="q-card-ia">
                ...
                <div class="q-content-scroll">
                    ...
                    <div id="q-loading-box">
                        <div class="q-loading-texts">
                            ...
                            <a href="https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=useaela" target="_blank" class="q-loading-t2">
                ...
                <a href="https://provoulevou.com.br?utm_source=widget&utm_medium=lojista&utm_campaign=useaela" target="_blank" class="q-powered-footer">
                    <span>Powered by</span>
    ...
    function init() {
            ...
            try {
                ...
                if (pending) {
                    ...
                    const resp = await fetch(WEBHOOK_PIX, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ email: 'cliente@provoulevou.com.br', phone, loja: 'useaela', origin: location.origin })
                    });
                    pix = await resp.json();
        ...
        async function runGeneration() {
            ...
            try {
                ...
                try {
                    // Guard: re-validar telefone antes de submeter (evita whatsapp vazio chegar ao backend)
                    const _finalNums = (phoneInput.value || '').replace(/\D/g, '');
                    if (!isValidBRPhone(_finalNums)) {
                        document.getElementById('q-loading-box').style.display = 'none';
                        uploadStep.style.display = 'block';
                        genBtn.disabled = false;
                        flashError(phoneInput, 'Preencha um WhatsApp válido para continuar');
                        return;
                    }
                    const fd = new FormData();
                    fd.append('person_image', userPhoto, 'person.jpg');
                    fd.append('whatsapp', '55' + _finalNums);
                    ...
                    console.log('[PL Aela] Enviando', allProdImgs.length, 'fotos do produto');
